...
async function main() {
  ...
  let categories = await prisma.incidentCategory.findMany({ where: { isActive: true } });
  if (categories.length === 0) {
    console.log("Creating default incident categories...");
    await prisma.incidentCategory.createMany({
      data: [
        { name: "Technical Issue", description: "Problems with software or hardware" },
        { name: "HR Complaint", description: "Human Resources related issues" },
        { name: "Building Maintenance", description: "Issues with the physical premises" },
      ],
    });
    categories = await prisma.incidentCategory.findMany({ where: { isActive: true } }); // Re-fetch after creation
    console.log(`✅ Created ${categories.length} default active categories`);
  }
  ...
  if (users.length === 0 && departments.length > 0) {
    ...
    await prisma.user.createMany({
      data: [
        { firstName: "Alice", lastName: "Smith", email: "alice@example.com", username: "alice", departmentId: departments[0].id, role: "admin" },
        { firstName: "Bob", lastName: "Johnson", email: "bob@example.com", username: "bob", departmentId: departments[1].id, role: "user" },
        { firstName: "Charlie", lastName: "Brown", email: "charlie@example.com", username: "charlie", departmentId: departments[0].id, role: "user" },
      ],
    });
    users = await prisma.user.findMany(); // Re-fetch after creation
    console.log(`✅ Created ${users.length} default users`);
  } else if (users.length === 0) {
    console.warn("⚠️ Cannot create default users: No departments available. Please ensure departments are seeded.");
  }
  ...
  const incidentsData = Array.from({ length: 20 }, (_, i) => {
    ...
    const reporter = users.length > 0 ? users[Math.floor(Math.random() * users.length)] : null;
    const assignee = users.length > 0 && Math.random() > 0.5 ? users[Math.floor(Math.random() * users.length)] : null; // Assignee is optional
    ...
    return {
      ...
      locationAddress: ["Addis Ababa Office", "Adama Branch", "Hawassa Data Center", "Dire Dawa Warehouse"][i % 4],
  ...
  const validIncidentsData = incidentsData.filter(inc => inc.reporterId !== undefined);
  ...
  console.log(`✅ Seeded ${validIncidentsData.length} incidents`);
}
