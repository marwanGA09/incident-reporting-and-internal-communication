...
import {
  ...
  useIncidentUIForm,
} from "./IncidentFormStore";
import { Step4Schema } from "@/lib/validation/incidents";

interface Step4FormProps {
  departments: { id: string; name: string }[];
  users: { id: string; name: string; position: string | null; departmentId: string | null }[];
}

export default function Step4Form({ departments, users }: Step4FormProps) {
  ...
  function onSubmit(values: z.infer<typeof Step4Schema>) {
    ...
    const assigneeName = users.find((user) => user.id === values.assigneeId)?.name;
