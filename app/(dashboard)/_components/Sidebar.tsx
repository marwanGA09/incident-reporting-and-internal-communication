...
function AppSidebarUI({
  ...
  return (
    // <aside className={cn("h-full flex-col border-r")}>
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              {groupsDepartmentLink.map((item) => {
                const count = getUnreadCountByUrl(item.url);
                const Icon = iconMap[item.icon];
                return (
                  <SidebarMenuItem key={item.title}>
                    <SidebarMenuButton asChild>
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              ...
              {usersFromDB.map((dbUser) => {
                ...
                const count = getUnreadCountByUrl(url);
                return (
...
export function AppSidebar(props: any) {
  const { getUnreadCountByBaseUrl, getUnreadCountByUrl } =
    useNotificationStore();
  const incidentNotificationCount = getUnreadCountByBaseUrl("/incidents");

  return (
    <AppSidebarUI
      {...props}
      incidentNotificationCount={incidentNotificationCount}
      getUnreadCountByUrl={getUnreadCountByUrl}
    />
  );
