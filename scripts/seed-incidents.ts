...
function generateIncidentDetails(departmentName: string, categoryName: string) {
  ...
  const titles =
    departmentSpecificTitles[departmentName as keyof typeof departmentSpecificTitles] || [
...
async function createStatusNotesAndSetFinalStatus(
  ...
  for (let i = 0; i < numTransitions; i++) {
    ...
    changedAt.setMinutes(changedAt.getMinutes() + faker.number.int({ min: 0, max: 59 }));
  ...
  console.log(`Created ${numTransitions} status notes for incident ${incidentId}, final status: ${lastStatus}`);
}
...
async function seedIncidents() {
  ...
  try {
    ...
    console.log('Cleared existing incident-related data.');
    ...
    if (departments.length === 0 || categories.length === 0) {
      throw new Error('No departments or categories found. Please run the metadata seeder first.');
    }

    console.log(`Found ${users.length} users, ${departments.length} departments, and ${categories.length} categories.`);

    // 3. Generate incidents for each department
    for (const dept of departments) {
      console.log(`Creating 3 incidents for department: ${dept.name}`);
    ...
    console.error('Error during incident seeding:', error);
    throw error;
  } finally {
...
seedIncidents()
  .then(() => console.log('Incident seeding process finished.'))
  .catch((error) => {
    console.error('Incident seeding failed:', error);
    process.exit(1);
  });
