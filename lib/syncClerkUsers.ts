 ...
 */
export async function syncClerkUsers(): Promise<{ created: number; total: number }> {
  try {
    ...
    console.log(`Found ${totalCount} users in Clerk (received ${clerkUsers.length} in first batch)`);
      ...
      if (!existingUser) {
        ...
        await prisma.user.create({
          data: {
            ...
            role: 'user', // Default role - you may want to customize this based on your needs
            position: 'lower', // Default position - you may want to customize this
          }
        });
        
        createdCount++;
        console.log(`Created user: ${clerkUser.firstName} ${clerkUser.lastName} (ID: ${clerkUser.id})`);
      } else {
        console.log(`User already exists: ${clerkUser.firstName} ${clerkUser.lastName} (ID: ${clerkUser.id})`);
      }
    }

    console.log(`Successfully synchronized users. ${createdCount} users created out of ${totalCount} total users.`);
 ...
 */
export async function syncClerkUsersWithPagination(): Promise<{ created: number; total: number }> {
  try {
    console.log('Starting user synchronization from Clerk to database (with pagination)...');
    ...
    while (true) {
      ...
      console.log(`Fetched batch: ${clerkUsers.length} users (offset: ${offset})`);
    ...
    console.log(`Successfully synchronized all users. ${totalCreated} users created out of ${totalProcessed} total users.`);
